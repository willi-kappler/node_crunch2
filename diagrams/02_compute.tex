...
\begin{tikzpicture}
    ...
    \matrix [column sep = 35mm, row sep = 5mm] {
        ...
        \node (prepare) [virtual] {nc\_get\_new\_data()}; &
        \node (node_process) [virtual] {nc\_process\_data()}; \\
        ...
        \node (server_process) [virtual] {nc\_process\_result()}; &
        \node (result_ok) [action] {result ok}; \\
    };
    ...
    \draw [action_step] (connect2) -- (result_ok);
    \draw [action_step] (result_ok.south) |- ++(3.2, -0.4) |- (loop.east);

    \draw [node_message] (connect1) -- node [above] {NodeNeedsMoreData} (connected1);
    \draw [server_message] (prepare) -- node [above] {NewDataFromServer} (node_process);
    ...
    \draw [node_message] (connect2) -- node [above] {NewResultFromNode} (connected2);
    \draw [action_step] (connected2) -- (server_process);
    % ResultOK
    \draw [action_step] (server_process.south) |- ++(-3.5, -0.4) |- (wait.west);
    \draw [server_message] (server_process) -- node [above] {ResultOK} (result_ok);
