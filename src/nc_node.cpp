...
void NCNode::nc_run() {
    nc_log_info("NCNode::nc_run() - starting node");
    ...
    while (!quit.load()) {
        try {
            switch (run_state) {
                ...
                default:
                    ...
                    nc_log_error(fmt::format("Unknown state: {}", static_cast<uint8_t>(run_state)));
                    quit.store(true);
                    continue;
            }
        ...
        switch (result.msg_type) {
            ...
            case NCServerMessageType::NewDataFromServer:
                ...
                nc_log_debug("New data from server.");
            ...
            case NCServerMessageType::ResultOK:
                ...
                // Request more data.
                nc_log_debug("ResultOK from server.");
                run_state = NCRunState::NeedData;
            break;
...
void NCNode::nc_send_heartbeat() {
    nc_log_info("NCNode::nc_send_heartbeat() - starting heartbeat thread.");
